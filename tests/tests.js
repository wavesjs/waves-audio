...
describe("Scheduler test", function() {
  ...
  beforeEach( function(){
    ...
    myObject = {
      makeAtNextTimeValue : 0,
      computeNextEventTimeValue : 0,
      getNextTime : function() {
        console.log("getNextTime");
      },
      makeAtNextTime : function() {
        myObject.makeAtNextTimeValue++;
      },
      computeNextEventTime : function() {
        myObject.computeNextEventTimeValue++;
      }
    };
  });
  ...
  it('should my scheduler is running correctly', function(done){
    ...
    setTimeout(function(){
      assert.equal(myObject.makeAtNextTimeValue, 2);
      assert.equal(myObject.computeNextEventTimeValue, 2);
